...
function mapUser(user: { id: string; name: string; email: string; avatarUrl: string | null; createdAt: Date }) {
  return {
    id: user.id,
    name: user.name,
    email: user.email,
...
const authRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post("/login", { config: { rateLimit: { max: 10, timeWindow: "1 minute" } } }, async (request) => {
  ...
  fastify.post("/refresh", { config: { rateLimit: { max: 20, timeWindow: "1 minute" } } }, async (request) => {
    ...
    const nextRefresh = authService.issueRefreshToken(env.REFRESH_TOKEN_TTL_DAYS);
    const accessToken = authService.issueAccessToken(refreshToken.user.id, refreshToken.user.email);
  ...
  fastify.post("/forgot-password", { config: { rateLimit: { max: 5, timeWindow: "1 minute" } } }, async (request) => {
    ...
    return {
      ...
      data: {
        ...
        delivery: "mock",
        accepted: true,
        resetTokenMock: user ? `mock-reset-${user.id}` : null
      }
    };
  });
};
