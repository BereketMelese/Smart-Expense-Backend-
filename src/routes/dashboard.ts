...
function getPeriodRange(targetType: HabitTargetType, now = new Date()): { start: Date; end: Date } {
...
function getCurrentStreak(dates: Date[]): number {
  const dateSet = new Set(dates.map((d) => new Date(d.getFullYear(), d.getMonth(), d.getDate()).getTime()));
...
const dashboardRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.get("/summary", { preHandler: fastify.authenticate }, async (request) => {
    ...
    const [monthlyAggregate, totalAggregate, checkIns] = await Promise.all([
    ...
    const monthlySpend = Number(monthlyAggregate._sum.amount ?? 0);
    const totalSpend = Number(totalAggregate._sum.amount ?? 0);
    const currentBalance = env.DEFAULT_START_BALANCE - totalSpend;
    ...
    return {
      ...
      data: {
        currentBalance,
        monthlySpend,
        currentStreak: getCurrentStreak(checkIns.map((c: any) => c.checkInDate))
      }
    };
  });

  fastify.get("/recent-transactions", { preHandler: fastify.authenticate }, async (request) => {
  ...
  fastify.get("/habit-progress", { preHandler: fastify.authenticate }, async (request) => {
    ...
    const progress = await Promise.all(
      habits.map(async (habit: any) => {
        ...
        const percent = Math.min(100, Math.round((current / habit.targetCount) * 100));
