...
const habitBodySchema = z.object({
  name: z.string().min(1),
  targetType: z.enum([HabitTargetType.DAILY, HabitTargetType.WEEKLY, HabitTargetType.MONTHLY]),
...
const habitsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post("/:id/check-ins", { preHandler: fastify.authenticate }, async (request, reply) => {
    const params = validateOrThrow(idParamSchema, request.params);
  ...
  fastify.get("/:id/check-ins", { preHandler: fastify.authenticate }, async (request) => {
    const params = validateOrThrow(idParamSchema, request.params);
